...
public class DBConnection implements StudentInterface {
    ...
    public boolean insertFacultyPersonalDetails(String id, GenderType gender,String dateOfBirth, String date, String district) throws SQLException {
        ps = con.prepareStatement("INSERT INTO  facultypersonaldetails(FacultyId, DOB, JoiningDate, Gender, District) VALUES(?,?,?,?,?)");
        ps.setString(1, id);
    ...
    public UserDetails getUser(String userId) throws SQLException {
        ...
        ps = con.prepareStatement("Select UserId, UserPassword,Role from userdetails where UserId = ? ");
        ps.setString(1, userId);
        rs = ps.executeQuery();
        while (rs.next()) {
            String passwrod = rs.getString("UserPassword");
            String role = rs.getString("Role");
            userDetails = new UserDetails(userId, passwrod,role);
        }
        return userDetails;
    }
